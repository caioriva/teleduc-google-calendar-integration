...
if ($etapa == 0) {
	...
	$content .= "<div class='formulario'>";
		$content .= "<form id='formInstall' name='formInstall' method='POST' action='index.php' onSubmit='return valida_form(document.formInstall, 2);'>";
			...
			$dir = getcwd();
			chdir('./../..');
			$content .= "<input class='form' type='text' size='25' name='arquivos' value='".getcwd()."/arquivos"."'/><br />";
			chdir($dir);
	...
	/*
	 Obtencao dos diretorios utilizados para nos comandos de restauracao da pasta de instalacao.
	 */
	$dir_atual = getcwd();
	chdir("../");
	$teleduc_webdriver = getcwd();
	chdir("../");
	$teleduc_config = getcwd()."/config";
	chdir($dir_atual);
	...
	$content .= "<p>A pasta de instala&ccedil;&atilde;o foi removida por quest&otilde;es de seguran&ccedil;a. &Eacute poss&iacutevel restaur&aacute-la com o comando abaixo:</p>";
	$content .= "<pre>";
	$content .= "cp -R ".$teleduc_config."/instalacao ".$teleduc_webdriver;
	$content .= "</pre>";
	$content .= "<br/>";
