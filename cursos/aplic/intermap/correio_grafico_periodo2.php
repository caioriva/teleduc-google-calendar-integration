...
-------------------------------------------------------------------------------
    ...
    Arquivo : cursos/aplic/intermap/correio_grafico_periodo2.php
...
/* C�digo principal */
  ...
  echo("  {\n");
  echo("    document.location='salvar_arquivo.php?".RetornaSessionID()."&cod_curso=".$cod_curso."&apresentacao=".$apresentacao."&inicio=".$inicio."&fim=".$fim."&agrupar=".$agrupar."&exibir=".$exibir."&nome_arquivo=correio_grafico_periodo'");
  echo("  }\n");
  ...
  echo("        <tr>\n");
  // 4 - a
  echo("          <td>".$inicio." ".RetornaFraseDaLista($lista_frases,4)." ".$fim."</td>\n");
  echo("          <td>\n");
  if ($agrupar=="dia")
    // 21 - Dia 
  ...
  else if ($exibir == "colaborador")
    // 84 - Somente Colaboradores
    echo("            ".RetornaFraseDaLista($lista_frases,84));
  echo("          </td>\n");
  echo("        </tr>\n");

  $msgs_qtde=RetornaQtdeMsgsPeriodo($sock,$cod_curso,$inicio,$fim,$agrupar);
  ...
  if (count($msgs_qtde)>0)
  {
     ...
     foreach($msgs_qtde as $cod => $linha)
     {
       if ($exibir=="todos")
         ...
         $msgs_qtde[$cod]['qtde']=(int)$linha['qtde_colaborador'];
     ...
     if ($agrupar=="dia" && $apresentacao=="grafico")
     {
       ...
       foreach($msgs_qtde as $periodo => $linha)
       {
         ...
         switch($exibir)
         {
           case "todos"       : $imagem = "todos.gif";        break;
           case "aluno"       : $imagem = "aluno.gif";        break;
           case "formador"    : $imagem = "formador.gif";     break;
           case "colaborador" : $imagem = "colaborador.jpeg"; break;
         }

         echo("          <td class=\"text\">".$linha['qtde']."<br>\n");
         echo("            <img src=\"figuras/".$imagem."\" border=\"0\" width=\"5\" height=\"".$tam."\" >\n");
         echo("          </td>\n");
       }
       echo("        </tr>\n");
     ...
     else if ($agrupar=="dia" && $apresentacao=="tabela")
     {
       ...
       // 46 - M�s
       echo("          <td>".RetornaFraseDaLista($lista_frases,46)."</td>\n");
     ...
     else if ($agrupar=="mes")
     {
       ...
       foreach($msgs_qtde as $periodo => $linha)
       {
         ...
         if ($apresentacao=="grafico")
         {
           ...
           echo("          <td align=\"left\">\n");
           echo("            <img src=\"figuras/".$imagem."\" border=\"0\" width=\"".$tam."\" height=\"5\">&nbsp;".$linha['qtde']."\n");
           echo("          </td>\n");
         }
         else
         {
           $i = ($i + 1) % 2;
           echo("          <td class=\"text\" align=\"center\">".$linha['qtde']."</td>\n");
         }
         echo("        </tr>\n");
       }
     }
     ...
     else
     {
       echo("        <tr class=\"head01\">\n");
       // 76 - Total de mensagens enviadas:
       echo("          <td colspan=\"3\">".RetornaFraseDaLista($lista_frases,76)."&nbsp;<a>".$total."</a></td>\n");
       echo("        </tr>\n");
     }
  }
  else
