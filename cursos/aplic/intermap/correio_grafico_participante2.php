...
/* C�digo principal */
  ...
  echo("  {\n");
  echo("    document.location='salvar_arquivo.php?".RetornaSessionID()."&cod_curso=".$cod_curso."&apresentacao=".$apresentacao."&cod_usu=".$cod_usu."&inicio=".$inicio."&fim=".$fim."&agrupar=".$agrupar."&nome_arquivo=correio_grafico_participante'");
  echo("  }\n");
  ...
  {
    // 72 - Todos
    echo("            ".RetornaFraseDaLista($lista_frases,72)."<br>\n");
  }
  else
  {
    echo("            ".MontaLinkPerfil($cod_usu,$lista_usuarios[$cod_usu])."<br>\n");
  }
  echo("          </td>\n");
  ...
  if ($cod_usu>=0)
  {
    $msgs_qtde=RetornaQtdeMsgsPeriodoUsuario($sock,$inicio,$fim,$agrupar,$cod_usu);
    ...
    if (count($msgs_qtde)>0)
    {
       ...
       if ($agrupar=="dia" && $apresentacao=="grafico")
       {
         ...
         echo("          <td colspan=\"".$number."\">".RetornaFraseDaLista($lista_frases,76)."&nbsp<a>".$total."</a></td>\n");
         echo("        </tr>\n");
       }
       else
       {
  ...
  else  // Todos os participantes: cod_usu=-1
  {
    ...
    $usu_msgs_qtde=RetornaQtdeMsgsPeriodoListaUsuarios($sock,$inicio,$fim,$agrupar,$lista_usuarios);
    $primeira_linha=true;
